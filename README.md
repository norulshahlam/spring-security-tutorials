...
2. get the public key + crtificate by running another script
...
# v5 - Store Tokens in DB

### previously we r using inMemory to store tokens generated. But this token is cleared everytime we restart the app. To persist the toekn, we will store the toekn in db. 

1. start your docker mysql

2. Create table for token. Once creeatewd, spring will auto save the token generated in this new table. This table can also be found in coupon-service\src\main\resources\tokentables.sql
...
3. replace the InMemoryTokenStore() in AuthorizationServerConfig to store into db

4. Generated token using postman:
  ...
  You will get token as the response. Now this time the toekn is stored in your db.

4. check if token is in db
  ...
  if the result is too long, u can query only selected fields:

    select token_id, authentication_id,  user_name, client_id, refresh_token  from oauth_access_token;

    select token_id from oauth_refresh_token;
